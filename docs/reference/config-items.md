...
| nodeName | string | empty | the name of the node where edgemesh-agent is scheduled. Prohibit user manual configuration, automatically recognized by the program |
| security | object | [Table 1-4-4-1](#t1-4-4-1) | tunnel acl configuration |
...
| token | string | auto fetch from  `tokensecret` secret in kubeedge namespace  | equivalent to token in edgecore.yaml |
